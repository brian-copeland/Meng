...
class GUI:
	...
	def step(self, data=None):
		...
		for j in range(0, len(self.world.phoneList)):
			newGuess = complex(0,0)
			points = []
			# print newGuess

			for i in range(0, len(self.world.routerList)):
				#Use complex data, u + vi
				# print stepData[j][i][0]
				newGuess += cmath.exp(complex(0, stepData[j][i][0]))
			...
			# print phone1Location
			# a = cmath.polar(phone1Location)
			# print a
			for i in range(0, len(self.world.routerList)):
				...
				print cmath.polar(newGuess)[1]
				self.world.phoneSoftwareList[j].otherGuess[i] = cmath.polar(newGuess)[1]
			...
			print newGuess
			self.draw_unfilled_circle("purple", phone1Location.x + newGuess.real, phone1Location.y + newGuess.imag)
